/**
 * Generates public/korrio-loading-v1-1.json — "the sign talks" (v1.1).
 *
 * Identical to v1 — the neon flicker-on and hum — but the mouth moves:
 * in the loop the lips chatter in two little talking bursts, scaling
 * about a near-top anchor so the mouth opens downward like a jaw.
 *
...
const layer = (ind, id, oKeys, ksExtra = {}) => {
...
// talking: two chatter bursts per loop, mouth hinged near its top edge
const OPEN = S * 1.45;
const CLOSED = S * 0.72;
const talkKeys = [
  [90, [S, S, 100], easeInOut],
  [96, [S, S, 100], easeInOut],
  [99, [S, OPEN, 100], easeInOut],
  [102, [S, CLOSED, 100], easeInOut],
  [105, [S, OPEN * 0.95, 100], easeInOut],
  [108, [S, CLOSED * 1.1, 100], easeInOut],
  [111, [S, OPEN * 0.97, 100], easeInOut],
  [114, [S, CLOSED, 100], easeInOut],
  [117, [S, OPEN * 0.9, 100], easeInOut],
  [121, [S, S, 100], easeInOut],
  [148, [S, S, 100], easeInOut],
  [151, [S, OPEN * 0.96, 100], easeInOut],
  [154, [S, CLOSED, 100], easeInOut],
  [157, [S, OPEN * 0.93, 100], easeInOut],
  [160, [S, CLOSED * 1.08, 100], easeInOut],
  [163, [S, OPEN * 0.97, 100], easeInOut],
  [166, [S, CLOSED, 100], easeInOut],
  [169, [S, OPEN * 0.88, 100], easeInOut],
  [173, [S, S, 100], easeInOut],
  [210, [S, S, 100]],
];
...
const layers = [
  ...
  layer(7, 'lips', popOn(44), {
    s: anim(talkKeys),
    // hinge near the top lip so opening drops the jaw, not the whole mouth
    a: still([45 / 2, 6, 0]),
    p: still([370.3, 364.4, 0]),
  }),
...
const out = join(root, 'public/korrio-loading-v1-1.json');
